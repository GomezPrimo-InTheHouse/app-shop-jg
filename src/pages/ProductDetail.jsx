...
import { useAuth } from "../context/AuthContext.jsx";
import { registrarVisualizacionApi } from "../api/shopApi.js";
import { Heart, Loader2, Share2, ShoppingCart, ArrowLeft } from "lucide-react";
...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    registrarVisualizacionApi({
      ...
      sesion_cliente_id: sesionId || null,
      origen: "web_shop",
    }).catch((err) => console.error("Error registrando visualización", err));
  }, [product?.id]);
  ...
  // Consulta directa al negocio por WhatsApp
  ...
  // Compartir el link del producto con otro contacto
  ...
  return (
    <BaseLayout>
      <main className="mx-auto max-w-5xl px-4 py-6 sm:py-10">
        ...
        <Link
          to="/"
          className="mb-5 inline-flex items-center gap-1.5 text-xs text-slate-400 transition hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300"
        >
        ...
        <div className="grid grid-cols-1 overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 lg:grid-cols-2">
          ...
          <div className="flex flex-col justify-between p-6 sm:p-8">
            <div>
              ...
              {/* Nombre */}
              <h1 className="text-xl font-medium leading-snug text-slate-900 sm:text-2xl dark:text-slate-50">
                {product.nombre}
              </h1>
            ...
            <div className="mt-8 flex flex-col gap-3">
              ...
              <button
                onClick={handleAddToCart}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-medium text-white transition hover:bg-slate-700 active:scale-[0.98] dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100"
              >
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {/* CONSULTAR: verde WhatsApp, escribe al negocio */}
                <button
                  onClick={handleConsultarWhatsApp}
                  className="flex items-center justify-center gap-2 rounded-xl bg-[#25D366] px-4 py-2.5 text-sm font-medium text-white transition hover:bg-[#1ebe5d] active:scale-[0.98]"
                >
                ...
                {/* COMPARTIR: outline neutro, envía link a un contacto */}
                <button
                  onClick={handleCompartirWhatsApp}
                  className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition active:scale-[0.98]
                    ${shared
              ...
              <p className="text-center text-xs text-slate-400 dark:text-slate-600">
                ...
                <span className="font-medium text-slate-500 dark:text-slate-400">Compartir</span> envía el link a un contacto
              </p>
